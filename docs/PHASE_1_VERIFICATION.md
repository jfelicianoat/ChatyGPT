...
| Aviso de ejecución | Verificado automáticamente | detector conservador para `ejecutar`, `compilar`, `probar` y equivalentes ingleses | test TypeScript `sandbox intent` | evita enviar silenciosamente una petición de pruebas sin `run_code` | requiere que el Broker publique el sandbox | escribir «crea el programa, ejecútalo y pruébalo» |
| Pool local y cloud | Verificado automáticamente | los chats normales permiten `ollama`, `lmstudio`, `nvidia` y `deepseek`, con `cloud_allowed=true` y coste máximo cero | test Rust `chat_routing_allows_local_and_cloud_providers_for_normal_context` | AI Broker puede elegir entre modelos locales, Ollama cloud y NVIDIA NIM; DeepSeek queda disponible cuando se habilite | la exploración efectiva requiere Broker actualizado | enviar varios mensajes y observar proveedor/modelo bajo cada respuesta |
| Memoria sensible local | Verificado automáticamente | cualquier recuerdo con sensibilidad `sensitive` reduce el pool a Ollama/LM Studio y fuerza `local_only` | test Rust `chat_routing_keeps_sensitive_memory_local_only` | el contexto marcado como sensible no sale a proveedores externos | depende de que el recuerdo esté correctamente clasificado | crear un recuerdo sensible, usarlo en un chat y comprobar un proveedor local |
| Modelo visible | Verificado automáticamente | `model_used` se proyecta desde la tarea a la respuesta sin exponer el payload | test Rust de materialización de respuesta | proveedor y modelo aparecen bajo el mensaje | depende de que Broker devuelva `model_used` | revisar una respuesta nueva |
