...
| Pool local y cloud | Verificado automáticamente | los chats normales declaran `risk.data_classification=internal` y omiten listas fijas de proveedores | test Rust `chat_routing_delegates_provider_selection_for_internal_context` | AI Broker 2.6 puede elegir entre todos los proveedores locales y cloud configurados | la selección efectiva depende de disponibilidad y ranking del Broker | enviar varios mensajes y observar proveedor/modelo bajo cada respuesta |
| Memoria sensible local | Verificado automáticamente | cualquier recuerdo con sensibilidad `sensitive` fuerza `risk.data_classification=local_only` | test Rust `chat_routing_keeps_sensitive_memory_local_only` | la frontera derivada 2.6 impide que ese contexto salga a proveedores externos | depende de que el recuerdo esté correctamente clasificado | crear un recuerdo sensible, usarlo en un chat y comprobar un proveedor local |
